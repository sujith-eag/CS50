# packages starting at 5h 36m
# 3rd party libraries, called packages
# it can be gotten at "pypi.org"  python package index

# we can downoad and place them or use inbild package managers of the language
...
# APIs,  Application Programing interface,  not general to python
# 3rd part services that we can talk to and download data to our system
...
for list in data["results"]:     # "results" is the key word in the json file from itune which has the list having "trackName"
    print( list["trackName"] )   # list gets only the list of trackName with songs as values
